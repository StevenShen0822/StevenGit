trigger OpportunityTrigger on Opportunity (before insert, before update, after insert, after update) {
    ...
    if(Trigger.isInsert && Trigger.isBefore) {
        ...
        for(Integer i = 0; i < Trigger.new.size(); i++) {
            ...
            if (oppRecordTypeName == 'Local' && String.isBlank(currentOpp.Type)) {
                currentOpp.Type = 'New';
            }    
        }
    }
    ...
    */
    ...
    // store the client approved opportunity to check if needs onboarding case flag
    Map<Id,Opportunity> mapIdToClientApprovedOppy = new Map<Id,Opportunity>();
    // store the AccountId and closed won opportunity Id to generate onboarding case
    Map<Id,Id> mapAccountIdToClosedWonOppyId = new Map<Id,Id>();
    ...
    if(trigger.isUpdate) {
        ...
        List<User> userList = [Select Id, Profile.Name, Alias From User Where Id = :UserInfo.getUserId()];
        for(Integer i = 0; i < trigger.new.size(); i++){
            ...
            if(rtMapById.get(record.RecordTypeId).getName().containsIgnoreCase('Local')){               
                if (trigger.isBefore){
                    ...
                    if(oldRecord.StageName == 'Closed Won' || oldRecord.StageName == 'Closed Lost'){
                        ...
                        if(userList.size() > 0 && userList[0].Profile.Name != 'System Administrator' && userList[0].Alias != 'hubsvc') {
                    ...
                    // when opportunity has been set to 'Closed Won', we check if needs Onboarding case and mark flag
                    if (oldRecord.StageName != 'Client Approved' && record.StageName == 'Client Approved') {
                        mapIdToClientApprovedOppy.put(record.Id, record);
                    }
                ...
                if (trigger.isAfter){
                    ...
                    // when opportunity has been set to 'Closed Won'and Onboarding_Required__c is true, we create Onboarding case
                    if (oldRecord.StageName != 'Closed Won' && record.StageName == 'Closed Won' && record.Onboarding_Required__c) {

                        mapAccountIdToClosedWonOppyId.put(record.AccountId, record.Id);
                    }
        ...
        if (!oppIDAndExpirationDateMap.isEmpty()) {
            ...
            /**
             * Allow partial success in Opportunity trigger so that the opportunity update would not be interrupted by validation rule error on Quote record 
             * and system will save the exception as a XO_Exception__c record.
             */
            if (!quoteUpdateList.isEmpty()) {
                Database.SaveResult[] srListNew = Database.update(quoteUpdateList, false);
                List<XO_Exception__c> xoExceptionList = ExceptionUtility.consumeException(srListNew);
        ...
        //When opportunity is set as 'Client Approved', check if needs Onboarding case and mark the flag
        if (!mapIdToClientApprovedOppy.isEmpty()) {
            //if an Opporutniy has been closed won, we should create an Onboarding Case when the opporunity meets the following conditions:
            ...
            Map<Id,List<Opportunity>> mapAcctIdToOppyNeedOnboarding = new Map<Id,List<Opportunity>>();
            ...
            Set<Id> setOppyIdNeedOnboarding = new Set<Id>();
            Set<Id> setClientApprovedOppyIDs = mapIdToClientApprovedOppy.keySet();
            ...
            String qrpQueryString = 'SELECT Id, Name, zqu__Quote__r.zqu__Opportunity__c, zqu__Quote__r.zqu__Opportunity__r.AccountId '
                                   ...
                                   + 'WHERE zqu__Quote__r.zqu__Opportunity__c IN: setClientApprovedOppyIDs ';
            ...
            for (zqu__QuoteRatePlan__c zquQRP : Database.query(qrpQueryString)) {

                Id acctId = zquQRP.zqu__Quote__r.zqu__Opportunity__r.AccountId;
                Id oppyId = zquQRP.zqu__Quote__r.zqu__Opportunity__c;

                if(String.isNotBlank(acctId) && mapIdToClientApprovedOppy.containsKey(oppyId)){
                    Opportunity oppy = mapIdToClientApprovedOppy.get(oppyId);

                    setOppyIdNeedOnboarding.add(oppyId);

                    if(!mapAcctIdToOppyNeedOnboarding.containsKey(acctId)){
                        mapAcctIdToOppyNeedOnboarding.put(acctId, new List<Opportunity>());
                    }

                    mapAcctIdToOppyNeedOnboarding.get(acctId).add(oppy);
                }
                
            }

            // 2 the Account has not had services with us for > 1year
            Set<Id> setAcctToOnboardingIDs = mapAcctIdToOppyNeedOnboarding.keySet();
            Date lastYearDate = Date.today().addYears(-1);

            if(!setAcctToOnboardingIDs.isEmpty()){
                String spcQueryString = 'SELECT Id,Zuora__Subscription__c, Zuora__Subscription__r.Zuora__TermStartDate__c, '
                                        ...
                                        + 'AND Zuora__Subscription__r.Latest_Quote__r.zqu__Opportunity__c NOT IN: setOppyIdNeedOnboarding '
                ...
                for (Zuora__SubscriptionProductCharge__c currentSPC : subscriptionPCList) {
                    ...
                    if (currentSPC.Zuora__Subscription__r.Zuora__TermStartDate__c < currentSPC.Zuora__Subscription__r.Zuora__TermEndDate__c 
                    && mapAcctIdToOppyNeedOnboarding.containsKey(currentSPC.Zuora__Subscription__r.Zuora__Account__c)) {
                        
                        mapAcctIdToOppyNeedOnboarding.remove(currentSPC.Zuora__Subscription__r.Zuora__Account__c);
                    }
                }
            }

            //Mark Onboarding_Required__c
            if(!mapAcctIdToOppyNeedOnboarding.isEmpty()){
                for(Id acctId : mapAcctIdToOppyNeedOnboarding.keySet()){
                    
                    List<Opportunity> listOppyNeedOnboarding = mapAcctIdToOppyNeedOnboarding.get(acctId);

                    if(!listOppyNeedOnboarding.isEmpty()){

                        for(Opportunity oppyNeedOnboarding : listOppyNeedOnboarding){

                            oppyNeedOnboarding.Onboarding_Required__c = TRUE;
                        }
                    }
                }
            }
        ...
        //When opportunity is set as 'Closed Won', we create Onboarding case if needs
        if (!mapAccountIdToClosedWonOppyId.isEmpty()) {

                List<Case> listOnboardingCaseToInsert = CaseService.createOnboardingCase(mapAccountIdToClosedWonOppyId);

                if(!listOnboardingCaseToInsert.isEmpty()){
                    List<XO_Exception__c> xoExceptionList = new List<XO_Exception__c>();

                    //create onboarding case
                    List<Database.SaveResult> insertResultsList = Database.insert(listOnboardingCaseToInsert,FALSE);
                    xoExceptionList.addAll(ExceptionUtility.consumeException(insertResultsList));
    ...
    if(Trigger.isAfter && Trigger.isUpdate){
        ...
        for(Opportunity opp : trigger.new){
            ...
            if(!oldOpp.StageName.equalsIgnoreCase('Closed Lost') && opp.StageName.equalsIgnoreCase('Closed Lost')){
                if(!statusChangeAccSet.contains(opp.AccountId) && String.isNotBlank(opp.AccountId)){
                    statusChangeAccSet.add(opp.AccountId);
                }
            }
        }
        
        if(!statusChangeAccSet.isEmpty()){
