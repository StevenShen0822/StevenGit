({
  ...
  saveExit: function(component, event, helper) {
    var goNext = false;
    var saveForLater = true;
    helper.saveCase(component, goNext, false, false, saveForLater);
  },

  saveAndAssignOwner: function(component, event, helper) {
    
    var goNext = false;
    helper.saveCase(component, goNext, false, true, false);
  ...
  closeCaseSubmitOpp: function(component, event, helper) {
    helper.saveCase(component, true, true, false, false);
  },
  ...
  next: function(component, event, helper) {
    var goNext = true;
    helper.saveCase(component, goNext, false, false, false);
  },

  /**
   * Handler for change Reason clear cancel Reason Notes.
   */
  changeReason: function(component, event, helper) {
    var currentCase = component.get('v.currentCase');
    if(currentCase.reason !== 'Other') {
      currentCase.cancelReasonNotes = '';
      component.set('v.currentCase', currentCase);
    }
  },
  /**
   * Handler for change Outcome clear cancel savedBy.
  */
  changeOutcome: function(component, event, helper) {

    var currentCase = component.get('v.currentCase');
    console.log('currentCase.outcome',currentCase.outcome);
  ...
  checkNextButton: function(component, event, helper) {
    ...
    if (currentPage === 1) {
      ...
      nextDisabled = !currentCase || !currentCase.type || 
      !currentCase.reason || !currentCase.outcome || (currentCase.reason==='Other' && !currentCase.cancelReasonNotes);
